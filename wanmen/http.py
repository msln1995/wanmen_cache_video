...
def downloadTask(ts_url, download_path,i, headers):
    file_name = ts_url.split("/")[-1]
    print("开始缓存 %s" %file_name)
    start = datetime.datetime.now().replace(microsecond=0)
    try:
...
def downloadThreading(url,download_path,Authorization, x_time, xtoken):
    headers = {
        "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/99.0.4844.74 Safari/537.36",
        "orgin": "https://www.wanmen.org",
        "referer": "https://www.wanmen.org",
    ...
    for i in range(len(ts_urls)):
        ts_url = ts_urls[i]
        任务 = threading.Thread(target=downloadTask, args=(ts_url, download_path, i, headers) )
        taskArr.append(任务)
